...
exports.createBootcamp = asyncHandler(async (req, res, next) => {
  ...
  if (publishedBootcamp && req.user.role !== "admin") {
    return next(
      new ErrorResponse(
        `The user with ID of ${req.user.id} has alredy published a bootcamp`,
        400
      )
    );
  }
...
exports.updateBootcamp = asyncHandler(async (req, res, next) => {
  ...
  if (bootcamp.user.toString() !== req.user.id && req.user.role !== "admin") {
    return next(
      new ErrorResponse(`User ${req.params.id} is not authorized to update this`, 401)
    );
  }
...
exports.deleteBootcamp = asyncHandler(async (req, res, next) => {
  ...
  if (bootcamp.user.toString() !== req.user.id && req.user.role !== "admin") {
    return next(
      new ErrorResponse(`User ${req.params.id} is not authorized to delete this`, 401)
    );
  }
